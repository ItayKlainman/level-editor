...
{
    // Overnight batch producer for Bus Buddies. Loops BusBuddiesLevelGenerator,
    // keeps the survivors (solvable + APS in band + not a near-duplicate), and
    // writes each as a LevelDocument JSON + PNG thumbnail + stats JSON into a dated
    // staging folder for curation in the Batch Review window. Mirrors YAKBatchHarness.
    //
    // Runs inside the Editor via the menu or headless:
    //   Unity -batchmode -quit -executeMethod
    //     Hoppa.BusBuddies.Editor.BusBuddiesBatchHarness.RunHeadless
    public static class BusBuddiesBatchHarness
    {
        private const string ProfilePath = "Assets/BusBuddies/Data/Config/BusBuddiesProfile.asset";

        [MenuItem("Window/Hoppa/BusBuddies/Run Batch (20)")]
        ...
        public static string RunBatch(int keep, int maxAttempts, float? apsOverride, string stagingRoot)
        {
            ...
            if (profile == null) { Debug.LogError($"[BusBuddiesBatch] Profile not found at {ProfilePath}"); return null; }
            if (profile.LevelGenerator == null || profile.LevelAnalyzer == null)
            {
                Debug.LogError("[BusBuddiesBatch] Profile needs both a Level Generator and a Level Analyzer wired.");
                return null;
            }

            var cfg = profile.GeneratorConfig as BusBuddiesGeneratorConfig;
            ...
            while (kept < keep && attempts < maxAttempts)
            {
                ...
                catch (Exception e) { Debug.LogError($"[BusBuddiesBatch] generate threw: {e.Message}"); continue; }
                ...
                string id = $"busbuddies_gen_{kept:000}_{seed}";
                doc.LevelId = id;
            ...
            Debug.Log($"[BusBuddiesBatch] kept {kept}/{keep} (in {attempts} attempts) → {stagingDir}");
