...
{
    // Walks a BusBuddiesDifficultyCurveConfig and mass-produces numbered,
    // difficulty-scaled Bus Buddies levels through the EXISTING
    // generate→autofill→analyze pipeline. For each level it assembles a transient,
    // tier-configured copy of the wired GameProfile (via BusBuddiesTierProfileBuilder
    // — never mutates on-disk assets) and calls the unchanged
    // profile.LevelGenerator.Generate. Output is level_N.json (+ .png + stats)
    // routed through the existing BatchReviewWindow staging convention. Mirrors
    // YAKCurveBatchHarness (no complexity axis in Bus Buddies 1a).
    public static class BusBuddiesCurveBatchHarness
    {
        public static string RunCurve(BusBuddiesDifficultyCurveConfig curve, GameProfile baseProfile,
                                      int attemptsPerLevel, string stagingRoot)
        {
            ...
            if (errors.Count > 0)
            {
                Debug.LogError("[BusBuddiesCurve] invalid curve: " + string.Join("; ", errors));
                return null;
            }
            if (baseProfile.LevelGenerator == null || baseProfile.LevelAnalyzer == null)
            {
                Debug.LogError("[BusBuddiesCurve] profile needs a Level Generator and Analyzer wired.");
                return null;
            }
            ...
            string root = stagingRoot ?? Path.Combine(
                Directory.GetParent(Application.dataPath).FullName, "BusBuddies_Batch");
            ...
            for (int i = 1; i <= total; i++)
            {
                ...
                var tp = BusBuddiesTierProfileBuilder.Build(baseProfile, tier);
                try
                {
                    ...
                    if (best == null) { Debug.LogWarning($"[BusBuddiesCurve] L{i}: no candidate produced."); continue; }
            ...
            Debug.Log($"[BusBuddiesCurve] generated {total} levels → {stagingDir}");
