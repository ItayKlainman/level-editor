...
{
    // EditorPanelAsset rendered in the ⇅ Order tab. Reads the master
    // level_config.json written by YAKLevelExporter, lists levels as a
    // drag-reorderable list, and (on Apply Order) renumbers the JSON keys
    // 1..N in the new order. Per-row delete is gated by a confirmation
    // dialog and writes back immediately.
    //
    // Modelled on YarnLevelOrderPanel but YAK's master file has no
    // LevelRewardConfigs section, so this only manages LevelConfigs.
    [CreateAssetMenu(menuName = "Hoppa/YAK/Level Order Panel")]
    public sealed class YAKLevelOrderPanel : EditorPanelAsset
    {
        [SerializeField] private YAKLevelExporter _exporter;

        public override string PanelId => "yak.level-order";
        ...
        private void Reload()
        {
            ...
            if (_exporter == null)
            {
                Debug.LogWarning("[YAKLevelOrderPanel] No exporter assigned. Set the Exporter field on this asset.");
                return;
            }
            ...
            {
                Debug.LogWarning($"[YAKLevelOrderPanel] Master JSON not found at '{path}'.");
                return;
            }
            ...
            {
                Debug.LogWarning($"[YAKLevelOrderPanel] Could not parse master JSON: {ex.Message}");
                return;
            }
